...
abstract class Person_Basic_Meta_Box {
	...
	public static function get_person_basic_meta_data( int $post_id ) : array {
		...
		foreach ( $meta_keys as $meta_key ) {
			...
			$meta_value = Movie_Library_Metadata_API::get_person_meta( $post_id, $meta_key, true );
	...
	public static function add_meta_data_to_database( string $meta_key, string $meta_value, int $post_id ) : void {
		...
		if ( empty( $meta_value ) ) {
			Movie_Library_Metadata_API::delete_person_meta( $post_id, $meta_key );
			return;
		}

		// Update the metadata.
		Movie_Library_Metadata_API::update_person_meta( $post_id, $meta_key, $meta_value );
	}
